...
// โหมดการเล่น
export const clashModes = [
  { icon: "🗼", name: "Tower (ไต่ชั้น)", desc: "สู้บอทไปเรื่อย ๆ ชนะ = เลื่อนชั้น แพ้ = อยู่ชั้นเดิม ยิ่งชั้นสูงบอทยิ่งโหด เล่นได้ตลอดเวลา" },
  { icon: "⚔️", name: "Duel / FFA", desc: "ประลองกับผู้เล่นจริง 2–8 คนแบบ FFA เติมบอทถ้าคนน้อย" },
  { icon: "🏆", name: "ทัวร์นาเมนต์", desc: "เปิดทุกวัน 21:00 (เวลาไทย) — แข่งรวม ผู้ชนะได้ตำแหน่งแชมป์สะสมขึ้นกระดาน" },
];
...
export const powerBonuses = [
  ...
  {
    icon: "📦",
    title: "โบนัสจำนวนสะสม (ลดหลั่น)",
    desc: "ยิ่งตกปลาตัวเดิมได้เยอะ ยิ่งแรง แต่เพิ่มแบบลดหลั่น (diminishing) เข้าใกล้เพดาน +25% — สะสมได้สูงสุด 5,000 ตัว/ชนิด (≈ +24%) พร้อมเลเวลปลาสูงสุด Lv.19",
  },
];

// เลเวลปลา (fishLevelFromCount) — คำนวณจากจำนวนที่สะสมได้ของปลาตัวนั้น
// level = 1 + floor( 19 × (n-1) / ((n-1) + 150) )   [n = จำนวนที่มี]
// เลเวลผูกกับโบนัสจำนวนสะสมโดยตรง (ตัวเดียวกัน) — สูงสุดจริง Lv.19 (Lv.20 ต้องมีปลา ∞ ตัว จึงไม่ถึง)
export const fishLevelMeta = { max: 19, cap: 5000, k: 150 };

// count น้อยสุดที่ถึงแต่ละเลเวล (ดึงจากในเกมจริง)
export const fishLevels = [
  { level: 1, count: 1 },
  { level: 2, count: 10 },
  { level: 3, count: 19 },
  { level: 4, count: 30 },
...
// ตัวอย่างเส้นโค้งโบนัสจำนวนสะสม: +25% × (n-1) / ((n-1)+150), เพดานเก็บ 5,000 ตัว
export const countCurve = [
  { count: 50, bonus: 6 },
  { count: 100, bonus: 10 },
  { count: 500, bonus: 19 },
  { count: 1000, bonus: 22 },
  { count: 5000, bonus: 24, cap: true },
];
