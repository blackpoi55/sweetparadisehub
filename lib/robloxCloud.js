...
export async function getCouplesFull() {
  ...
  // ดึงชื่อ + avatar ตาม userId ตรง ๆ (กันชื่อ↔รูปสลับ เพราะ a/b ในทะเบียนไม่ได้เรียงตาม lo/hi)
  const [users, avatars] = await Promise.all([
    fetchRobloxUsersBasic(ids),
    fetchRobloxUserAvatars(ids),
  ]);
  ...
  for (const u of users) {
    if (!u) continue;
    const id = String(u.id);
    userMap[id] = { name: u.displayName || u.name, avatarUrl: avatars[id] || null };
  }
  for (const [id, url] of Object.entries(avatars)) {
    if (!userMap[id]) userMap[id] = { avatarUrl: url };
  }
