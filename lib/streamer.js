...
import { KIND_SPEC, cleanParams } from "@/json/streamerKinds";
...
export const KINDS = ["dance", "soak", "launch", "slap", "firework", "text", ...Object.keys(KIND_SPEC)];
...
export function sanitizeEffect(input, id) {
  ...
  } else if (KIND_SPEC[kind]) {
    params = cleanParams(kind, p); // ชุดใหม่: ตรวจตามสเปกใน json/streamerKinds.js
  }
  return {
    id: id || crypto.randomBytes(6).toString("base64url"),
    name:
      clampChars(String(input?.name ?? "").trim(), 40) ||
      KIND_LABEL[kind] ||
      `${KIND_SPEC[kind].icon} ${KIND_SPEC[kind].label}`,
    kind,
    params,
...
/** บอกเซิร์ฟที่สตรีมเมอร์อยู่ว่าใช้รหัสล็อกอินแล้ว → แอพในเกมเลิกโชว์รหัส/นับถอยหลัง (พลาดได้ แอพเช็คเองซ้ำทุก 5 วิ) */
export async function notifyLinked(uid) {
  await publish({ k: "linked", u: Number(uid) });
}
